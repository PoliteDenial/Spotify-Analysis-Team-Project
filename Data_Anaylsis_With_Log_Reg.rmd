...
get_metrics = function(Actual, Predicted, model, rf = 0){
  ...
  else {r_sq =mse/var(Actual)}
  #adj_r_sq = ((1-r_sq)*(nrow(train)-2)/(length(mf)-nrow(train)-2))*1
...
#best_lambda
#plot(cv_model)
elastic_model <- glmnet(x,y,alpha = 0, lambda = best_lambda)
coef(elastic_model)
#--------------------------------------------------------------------------------------------
```
```{r, warn =-1}
...
get_metrics(test$ranking,y_pred_rf, random_forest_model, 1)
```
...
```{r}
# View(train)
train <- train %>% mutate(trend_or_not = ifelse(ranking < 50, 1, 0))
# train
log_reg_model <- glm(
  trend_or_not ~ ., 
  data = train, 
