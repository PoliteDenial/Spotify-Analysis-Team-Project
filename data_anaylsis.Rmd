...
# random forest/logistic/linear = 2
...
getEstimatedRank = function(song_query = "", mf = "", modelObj,  model_type, lambda = 0)
{
  ...
  "2"=
  predict(modelObj, newx = new_track)
  )
}
#////////////////////////////////////////////////////////////////////////////////////////////////
...
get_metrics = function(Actual, Predicted, model){
  mse = mse(Actual,Predicted)
  rmse = rmse(Actual,Predicted)
  norm_rmse = rmse(Actual,Predicted) / (max(Actual)-min(Actual))
  tss = sum((Actual - mean(Actual))^2)
  regression_ss = sum((Predicted - mean(Predicted))^2)
  r_sq = regression_ss/tss
...
#best_lambda
#plot(cv_model)
elastic_model <- glmnet(x,y,alpha = 0.5, lambda = best_lambda)
coef(elastic_model)
...
get_metrics(test$ranking, y_predicted, elastic_model)
#elasticEval = ggplot(aes(y_predicted ))
```
...
print(importance(random_forest_model,type = 2)) 
...
get_metrics(y_pred_rf, test$ranking, random_forest_model)
...
#val = round(append(y_pred_rf,true_rank, index))
#category = rep(c("pred", "actual"), each = length(true_rank)
...
```{r}
#still needs work
getEstimatedRank("Taps", model_features , random_forest_model , 2)
```