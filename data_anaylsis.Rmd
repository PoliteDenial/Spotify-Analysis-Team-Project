...
Tree graphing function from [here](https://www.r-bloggers.com/2017/03/plotting-trees-from-random-forest-models-with-ggraph/) a
```{r}
library(ggraph)
library(igraph)
...
getEstimatedRank = function(song_query = "", mf = "", modelObj,  model_type = 0, lambda = 0)
{
  ...
  if(model_type == 1){
    ...
    l = as.vector(predict(modelObj, newx = new_track))
    return(mean(tail(l, n=20)))
  }else{
    cat("\nInvalid model type!","\nTry 1 or 2")
  }
...
As You can see our random forest model performs quite a bit better then the elastic net reggression and

```{r}

#Plot elastic regression true_rank vs avg rank
...
tree_func(random_forest_model, ) tree_plot

```

The function below allows you to search for a song in the spotify catalog and using one of the models that we created output the prediction of that particular song. This particular call of the function is set to use the random forest regression model.
```{r}
#Currently is broken since predict of a random forest returns alot of null variables
getEstimatedRank("Leo Trix", model_features , random_forest_model, 2)

```

With regards to our binary test of top X tracks we should make sure that the distribution of our test isn't skewed. After examining the below graph is it is obvious that the test set of the data was slightly skewed towards worse ranking tracks however, it really is only a slight s
```{r}
hist(test$ranking)
```
Extremely poor perfroming linear Regression Model that isn't even worth running
```{r}
